...
template <class T> class Task {
  public:
    ...
    void begin() {
        BaseType_t result =
            xTaskCreatePinnedToCore(taskFunction, name, stackDepth, this, priority, &taskHandle, coreId);
        assert("Failed to create task" && result == pdPASS);
    }

  protected:
    virtual void eventHandler(Event event); // ADDED BY carlhampuswall
    Event event;                            // ADDED BY carlhampuswall
